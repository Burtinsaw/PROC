...
export default function ExchangeRatesWidget() {
  ...
  const trend = arrow === '↑' ? 'up' : arrow === '↓' ? 'down' : 'neutral';
    return (
      <Chip
        ...
        // default'ta çerçevesiz, transparan dolgu görünümü
        variant={'filled'}
        label={
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            {/* Kod başta; çerçeve ile kod arası label paddinginden 2px */}
            <Box component="span" sx={{ fontWeight: 700, ml: 0 }}>{code}</Box>
            {/* Kod → değer arası (3px) */}
            <Box component="span" sx={{ opacity: 0.9, ml: '3px', pr: '1px' }}>
              {typeof value === 'number' ? rateFormatter.format(value) : '-'} {arrow}
            </Box>
            {/* Sparkline en sağda; çerçeveye 2px kalacak şekilde */}
            <Box sx={{ ml: '2px', mr: 0 }}>
  ...
  sx={{
          ...
          fontWeight: 600,
          minHeight: 'unset',
          bgcolor: color === 'default' ? 'transparent' : undefined,
          border: 'none',
          boxShadow: 'none',
          '& .MuiChip-label': {
            pl: '2px', pr: '2px', py: 0,
